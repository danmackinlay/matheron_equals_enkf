...
format:
  revealjs:
    ...
    logo: figures/csiro_logo.png
    # logo-height: 300px  # fruitless
    css: logo.css
...
## Striving for peace between ML and Geostats

![](/figures/Gaussian_Pixie_dust.jpeg)
...
::: {.notes}
EnKF is a workhorse in geoscience for data assimilation; Matheron / pathwise GP sampling is popular in ML and geostatistics.
We excavate the result that the EnKF analysis step is just an empirical Matheron update.
This connection is simple, but not made explicitly, so let's do so.
...
## Matheron update is SOTA

![](/figures/Borovitskiy2023Matern.jpg)

@Borovitskiy2023Matern

## Gaussian conditioning à la Kálmán
...
::: {.notes}
Standard Kalman posterior updated ("analysis") step.
...
## Kalman update as GP regression

For any discrete set of sites GP regression is just Gaussian conditioning with a sparse $\mm{H}$, e.g.
...
## Gaussian conditioning à la Evensen
...
x \mid (y = y^*)
\;\disteq\;
x + \mm{C}_{\rv{xy}} \mm{C}_{\rv{yy}}^{-1}(y^* - y).
$$
...
![](/figures/matheron_3panel_stage.png){ height=580px }
...
![](/figures/timing_vs_observations.png){ width=80% }

![](/figures/timing_vs_dimensions.png){ width=80% }
...
[https://github.com/danmackinlay/matheron_equals_enkf](https://github.com/danmackinlay/matheron_equals_enkf)

![](/figures/qrcode.png){ height=400px }
