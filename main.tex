...
\paragraph{Speed.}
Figure~\ref{fig:timing-curves} reports \textbf{fit} (training) and \textbf{predict} times as we sweep (top) the number of observations $m$ at fixed state dimension $d$, and (bottom) the state dimension $d$ at fixed $m$.
The exact GP (scikit-learn) shows the expected $\mathcal{O}(m^3)$ fit scaling. Its predict time for mean-only scales as $\mathcal{O}(d \cdot m)$.
In contrast, EnKF and LETKF fits scale roughly \textbf{linearly} in $m$ for fixed ensemble size $N$ (and linearly in $d$ with localization), while prediction is \textbf{linear in $d$} to read out the analysis mean.
